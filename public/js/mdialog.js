...
TipBox.prototype.renderUI = function(tipType) {
	...
	if (this.config.hasBtn) {
		...
		switch (this.config.type) {
			case 'success':
				$(".success").after("<button class='okoButton'>ok</button>");
				break;
			case 'error':
				$(".lose").after("<button class='okoButton redOkoButton'>ok</button>");
				break;
			case 'tip':
				$(".tip").after("<button class='okoButton'>ok</button>");
				break;
			default:
				break;
		}
...
TipBox.prototype.successRenderUI = function() {
	...
	suc += "</div>";
	suc += " <div class='dec_txt' style='font-size: 30px;'>" + this.config.str + "</div>";
	suc += "</div>";
...
TipBox.prototype.errorRenderUI = function() {
	...
	err += "       </div>";
	err += "   </div>";
	err += "<div class='dec_txt' style='font-size: 30px;'>" + this.config.str + "</div>";
	err += "</div>";
